 ...
 */
 export const useGet__PascalName__ = (...args) => {
  const dispatch = useDispatch();
 // parse the args to determine what was passed in, this allows us to use only an endpoint or only listArgs (with default endpoint) or both
 let endpoint = null;
 let listArgs = 'all';
 if(args.length === 1) {
   // check type of first arg
   if(typeof args[0] === 'string') {
     // if it's a string, it's the endpoint
     endpoint = args[0];
   } else {
     // otherwise it's the listArgs
     listArgs = args[0];
   }
 } else if(args.length === 2) {
   // if there are two args, the first is the endpoint and the second is the listArgs
   endpoint = args[0];
   listArgs = args[1];
 }
 // set up __camelName__ specific stuff to be used by the shared hook
 ...
 const fetchSingle__PascalName__ = (queryString) => fetchSingleIfNeeded(queryString, fetch__PascalName__);
 const sendFetchSingle = (queryString) => dispatch(fetchSingle__PascalName__(queryString));
