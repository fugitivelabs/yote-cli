...
// define and export the strings for the different specific product endpoints once here.
...
export const fetchSingle__PascalName__AtEndpoint = createAsyncThunk(
  ...
  , async (query) => {
    const endpoint = `/api/__kebabNamePlural__${query}`
    const response = await apiUtils.callAPI(endpoint);
    // The value we return becomes the `fulfilled` action payload
    return response;
  }
...
export const fetch__PascalName__ListAtEndpoint = createAsyncThunk(
  ...
  , async (query) => {
    const endpoint = `/api/__kebabNamePlural__${query}`; // example: `/api/__kebabNamePlural__/logged-in?${queryString}`
    const response = await apiUtils.callAPI(endpoint);
    // The value we return becomes the `fulfilled` action payload
    return response;
  }
