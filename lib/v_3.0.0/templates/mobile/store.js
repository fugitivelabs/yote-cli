...
export const fetchListIfNeeded = (queryKey) => (dispatch, getState) => {
  ...
  if(shouldFetch(__camelName__Query)) {
    ...
    return dispatch(fetch__PascalName__List(queryKey));
  } else {
    return dispatch(() => {
      // console.log('No need to fetch, fresh query in cache');
      const __camelName__Store = getState().__camelName__;
      const query = __camelName__Store.listQueries[queryKey];
      return Promise.resolve({
        payload: {
          __camelName__s: selectListItems(__camelName__Store, queryKey)
          , ...query
        }
      })
    })
...
export const fetchSingleIfNeeded = (id) => (dispatch, getState) => {
  ...
  if(shouldFetch(__camelName__Query)) {
    return dispatch(fetchSingle__PascalName__(id));
  } else {
    // console.log('No need to fetch, fresh query in cache');
    return dispatch(() => {
      // console.log('No need to fetch, fresh query in cache');
      ...
      return Promise.resolve({
        payload: {
          __camelName__: selectSingleById(__camelName__Store, id)
        }
      })
    })
  }
