...
export const useGet__PascalName__List = (listArgs = {}, forceFetch = false) => {
  ...
  // set up __camelName__ specific stuff to be used by the shared hook
  const __camelName__Store = useSelector(({ __camelName__ }) => __camelName__);
  const fetch__PascalName__s = forceFetch ? fetch__PascalName__List : fetchListIfNeeded;
  const sendFetchList = (queryString) => dispatch(fetch__PascalName__s(queryString));
