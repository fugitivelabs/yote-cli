...
export const useGet__PascalName__List = (listArgs = {}, forceFetch = false) => {
  ...
  // set up __camelName__ specific stuff to be used by the shared hook
  const __camelName__Store = useSelector(({ __camelName__ }) => __camelName__);
  const fetch__PascalNamePlural__ = forceFetch ? fetch__PascalName__List : fetchListIfNeeded;
  const sendFetchList = (queryString) => dispatch(fetch__PascalNamePlural__(queryString));
  const sendInvalidateList = (queryString) => dispatch(invalidateQuery(queryString));

  // return the (now __camelName__ specific) hook
  return useGetResourceList({ listArgs, fromStore: __camelName__Store, sendFetchList, sendInvalidateList });
}
 ...
 * @param {object} listArgs - an object used to construct the query
 * @param {boolean} forceFetch - optional override to force a fetch from the server
 * @returns an object containing fetch info and eventually the __camelName__Ids list (as `data`)
...
export const useInfinite__PascalName__List = (listArgs = {}, forceFetch = false) => {
  ...
  // return the (now __camelName__ specific) hook
  return useInfiniteResourceList({ listArgs, sendFetchList, sendInvalidateLists, listKey: '__camelNamePlural__' });
