...
const Infinite__PascalName__List = (props) => {
  ...
  // invalidate the accumulated data when the user navigates to the create __camelName__ screen so we can be sure the new one is loaded when they navigate back
  const goToCreate__PascalName__ = () => {
    infinite__PascalName__List.invalidate();
    navigation.navigate('Create__PascalName__');
  }

  return (
    <View style={tw`flex-1 bg-white`}>
      <YTHeader
        title="__PascalNamePlural__"
      // rightItem={rightItem}
      />
      <View style={tw`p-2`}>
        <View style={tw`p-2`}>
          <YTButton
            ...
            onPress={goToCreate__PascalName__}
          />
        </View>
        <View style={tw`mb-80`}>
          <InfiniteList
