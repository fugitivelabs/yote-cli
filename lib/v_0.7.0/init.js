...
module.exports = function(appName, options) {
  ...
  var proper =utils.capitalizeFirstLetter(appName);
  ...
  if(!installOptions.indexOf('mobile') < 0) {
    ...
    console.log();
    shell.cd("mobile");
    shell.exec("react-native init --version='0.41.0' " + appName);
    shell.sed('-i', 'Yote', appName, `./Yote/index.ios.js`);
    shell.sed('-i', 'Yote', appName, `./Yote/index.android.js`);
    shell.cp('-f', [`./Yote/index.ios.js`, `./Yote/index.android.js`, `./Yote/package.json` ], `./${appName}/`);
    shell.cp('-Rf', `./Yote/js/*`, `./${appName}/js`);
    shell.sed('-i', 'Yote', appName, `./${appName}/js/setup.js`);
    shell.sed('-i', 'Yote', appName, `./${appName}/js/setup.js`);
    shell.sed('-i', 'Yote', appName, `./${appName}/js/setup.js`);
    shell.sed('-i', `Yote`, appName, `./${appName}/js/YoteApp.js`);
    shell.sed('-i', `Yote`, appName, `./${appName}/js/YoteApp.js`);
    shell.mv(`./${appName}/js/YoteApp.js`, `./${appName}/js/${appName}App.js`);
    shell.rm('-rf', './Yote');
    shell.cd(appName);
    shell.exec('npm install');
