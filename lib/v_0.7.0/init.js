...
module.exports = function(appName, options) {
  ...
  var properName =utils.capitalizeFirstLetter(appName);
  ...
  if(!installOptions.indexOf('mobile') < 0) {
    ...
    console.log();
    shell.cd("mobile");
    shell.exec("react-native init --version='0.41.0' " + properName);
    shell.sed('-i', 'Yote', properName, `./Yote/index.ios.js`);
    shell.sed('-i', 'Yote', properName, `./Yote/index.android.js`);
    shell.cp('-f', [`./Yote/index.ios.js`, `./Yote/index.android.js`, `./Yote/package.json` ], `./${properName}/`);
    shell.cp('-Rf', `./Yote/js/*`, `./${properName}/js`);
    shell.sed('-i', 'Yote', properName, `./${properName}/js/setup.js`);
    shell.sed('-i', 'Yote', properName, `./${properName}/js/setup.js`);
    shell.sed('-i', 'Yote', properName, `./${properName}/js/setup.js`);
    shell.sed('-i', `Yote`, properName, `./${properName}/js/YoteApp.js`);
    shell.sed('-i', `Yote`, properName, `./${properName}/js/YoteApp.js`);
    shell.mv(`./${properName}/js/YoteApp.js`, `./${properName}/js/${properName}App.js`);
    shell.rm('-rf', './Yote');
    shell.cd(appName);
    shell.exec('npm install');
