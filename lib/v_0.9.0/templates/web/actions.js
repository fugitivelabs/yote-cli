...
export const fetchSingleIfNeeded = (id) => (dispatch, getState) => {
  if (shouldFetchSingle(getState(), id)) {
    ...
    return dispatch(returnSingle__PascalName__Promise(id)); // return promise that contains __camelName__
  }
}
...
export function fetchSingle__PascalName__ById(__camelName__Id) {
  return dispatch => {
    dispatch(requestSingle__PascalName__(__camelName__Id))
    return apiUtils.callAPI(`/api/__kebabNamePlural__/${__camelName__Id}`)
...
export function setSelected__PascalName__(item) {
  return {
...
function requestDefault__PascalName__(id) {
  return {
...
function receiveDefault__PascalName__(json) {
  return {
    error: json.message
...
export function fetchDefault__PascalName__() {
  return dispatch => {
    dispatch(requestDefault__PascalName__())
    return apiUtils.callAPI(`/api/__kebabNamePlural__/default`)
      .then(json => dispatch(receiveDefault__PascalName__(json)))
  }
}


export const REQUEST___actionCase___SCHEMA = "REQUEST___actionCase___SCHEMA";
function request__PascalName__Schema(id) {
  return {
...
function receive__PascalName__Schema(json) {
  return {
    error: json.message
    , schema: json.schema
    , receivedAt: Date.now()
...
export function fetch__PascalName__Schema() {
  return dispatch => {
    dispatch(request__PascalName__Schema())
    return apiUtils.callAPI(`/api/__kebabNamePlural__/schema`)
      .then(json => dispatch(receive__PascalName__Schema(json)))
  }
}


export const REQUEST_CREATE___actionCase__ = "REQUEST_CREATE___actionCase__";
function requestCreate__PascalName__(__camelName__) {
  return {
...
export function sendCreate__PascalName__(data) {
  return dispatch => {
    ...
    return apiUtils.callAPI('/api/__kebabNamePlural__', 'POST', data)
...
function requestUpdate__PascalName__(__camelName__) {
  return {
...
export function sendUpdate__PascalName__(data) {
  return dispatch => {
    ...
    return apiUtils.callAPI(`/api/__kebabNamePlural__/${data._id}`, 'PUT', data)
...
function requestDelete__PascalName__(id) {
  return {
...
function receiveDelete__PascalName__(id, json) {
  return {
    id
    , error: json.message
    , receivedAt: Date.now()
...
export function sendDelete(id) {
  return dispatch => {
    ...
    return apiUtils.callAPI(`/api/__kebabNamePlural__/${id}`, 'DELETE')
      .then(json => dispatch(receiveDelete__PascalName__(id, json)))
  }
}
...
export function add__PascalName__ToList(id, ...listArgs) {
  // console.log("Add __camelName__ to list", id);
  if(listArgs.length === 0) {
    listArgs = ["all"];
  }
  return {
...
export function fetchList(...listArgs) {
  return dispatch => {
     ...
     *
     * if listArgs has 1 arg, return "/api/__kebabNamePlural__/by-[ARG]"
     *
     * if 2 args, additional checks required.
     *  if 2nd arg is a string, return "/api/__kebabNamePlural__/by-[ARG1]/[ARG2]".
     *    ex: /api/__kebabNamePlural__/by-category/:category
     *  if 2nd arg is an array, though, return "/api/__kebabNamePlural__/by-[ARG1]-list" with additional query string
     *
     * TODO:  make this accept arbitrary number of args. Right now if more
     * than 2, it requires custom checks on server
     */
    let apiTarget = "/api/__kebabNamePlural__";
    if(listArgs.length == 1 && listArgs[0] !== "all") {
      ...
      // this can be used for querying for a list of __camelNamePlural__ given an array of __camelName__ id's, among other things
