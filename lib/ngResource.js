...
function buildNgResource() {
  ...
  // Build SASS file for resource
  var sassPath = "./public/sass/pages";
  mkdir(sassPath, function() {
    write(sassPath + '/' + name + '.scss', readTemplate('client/page.scss'));
  });
  ...
  // Build sample data for the resource
  var dataPath = "./public/app/data";
  mkdir(dataPath, function(){
    write(dataPath + '/' + name + '.json', readTemplate('client/sample-data.json'));
  })
